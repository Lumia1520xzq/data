...
public class DailyDataJob  {
	...
	@Autowired
	private EsUicBuryingPointService buryingPointService;
	@Autowired
	private EsUicPlatformService platService;
	...
	public void execute() {
		...
		while (count <= 5) {
			try {
				// 获取收件人 ------ 收件人要改
				String receivers = configRpcService.findByName(DataConstants.DAILY_DATA_RECEIVER).getValue();
