...
public class IpInfoListJob {
    ...
    public void execute() {
        logger.info("根据ip分析用户行为开始:traceId={}", TraceIdUtils.getTraceId());
        int ipCount = dataConfigService.getIntValueByName(DataConstants.MONITOR_IP_COUNT);
        if (ipCount == 0) {
            logger.error("根据ip分析用户行为iPCount未设置，traceId={}", TraceIdUtils.getTraceId());
            return;
        }
        ...
        if (ips.size() <= 0) {
            logger.info("同ip数量大于{},为空，traceId={}", ipCount, TraceIdUtils.getTraceId());
        ...
        logger.info("根据ip分析用户行为结束:traceId={}",TraceIdUtils.getTraceId());
    }
            ...
            if (StringUtils.isNotEmpty(receivers)) {
                ...
                for (String to : receivers.split(",")) {
                    try {
                        ...
                        logger.error("根据ip分析用户行为发送失败，ex={}，traceId={}", LogExceptionStackTrace.erroStackTrace(e), TraceIdUtils.getTraceId());
                    }
                }
                logger.info("根据ip分析用户行为发送成功:traceId={}", TraceIdUtils.getTraceId());
            } else {
                logger.error("根据ip分析用户行为接收人未设置，traceId={}", TraceIdUtils.getTraceId());
            }
        } catch (Exception e) {
            logger.error("根据ip分析用户行为异常，traceId={}", TraceIdUtils.getTraceId());
        }
    }
}
