Ext.define('WF.view.sys.execute.historyDataCleanMain', {
    ...
    initComponent: function () {
        ...
        me.add({
            ...
            store: store,
            xtype: 'searchpanel',
            title: '补全dataware_final_game_info',
            collapsible: true,
            collapsed: false,
            columns: 2,
            buildField: "Manual",
            forceFit: true,
            todoExec: function () {
                Ext.Msg.confirm("确认", "确定要补全dataware_final_game_info历史数据吗?", function (button) {
                    if (button == "yes") {
                        var data = {
                            startTime: Ext.util.Format.date(me.down("[name='startTime']").getValue(), 'Y-m-d'),
                            endTime: Ext.util.Format.date(me.down("[name='endTime']").getValue(), 'Y-m-d')
                        };
                        callapi("data/admin/dataClean/gameOverView.do", data, function (result) {
                            if (result.success) {
                                Ext.Msg.show({
                                    title: "提示",
                                    msg: result.data.msg,
        ...
        me.add({
            ...
            forceFit: true,
            todoExec: function () {
                Ext.Msg.confirm("确认", "确定要补全activityCost历史数据吗?", function (button) {
                    if (button == "yes") {
                        var data = {
                            startTime: Ext.util.Format.date(me.down("[name='actCostStart']").getValue(), 'Y-m-d'),
                            endTime: Ext.util.Format.date(me.down("[name='actCostEnd']").getValue(), 'Y-m-d')
                        };
                        callapi("data/admin/dataClean/activityCost.do", data, function (result) {
                            if (result.success) {
                                Ext.Msg.show({
                                    title: "提示",
                                    msg: result.data.msg,
        ...
        me.add({
            ...
            forceFit: true,
            todoExec: function () {
                Ext.Msg.confirm("确认", "确定要补全rechargeStatistic历史数据吗?", function (button) {
                    if (button == "yes") {
                        var data = {
                            startTime: Ext.util.Format.date(me.down("[name='regStatStart']").getValue(), 'Y-m-d'),
                            endTime: Ext.util.Format.date(me.down("[name='regStatEnd']").getValue(), 'Y-m-d')
                        };
                        callapi("data/admin/dataClean/rechargeStatistic.do", data, function (result) {
                            if (result.success) {
                                Ext.Msg.show({
                                    title: "提示",
                                    msg: result.data.msg,
